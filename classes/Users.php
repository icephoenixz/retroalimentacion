         ...
         if(strlen($this->role) > 0){
          ...
          $query = "UPDATE users SET fname='$this->fname',lname='$this->lname',gender='$this->gender',address='$this->address',"
            ...
            if($connection->allUpdateInsertDelete($query)){
                ...
                $type = "success";
                $msg = "<strong>Success !</strong> registro actualizado  !";
                Session::setMessage($type, $msg);
            }else{
                $type = "danger";
                $msg = "<strong>Error !</strong> No se puede actualizar el registro  !";
                Session::setMessage($type, $msg);
            }
    }
    ...
    public function forgotPassword(){
        ...
        if($result->num_rows > 0){
            ...
            if($this->updatePassword()){
                $type = "success";
                $msg = "Cambio Exitoso ! </strong> Tu contraseña es <font style='color:black'>".$hased."</font>";
                Session::setMessage($type, $msg);
            }else{
                $type = "danger";
                $msg = "<strong>Error !</strong> No es posible recuperar la contraseña";
                Session::setMessage($type, $msg);
            }
        }else{
            $type = "danger";
